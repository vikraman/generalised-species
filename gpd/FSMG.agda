...
module _ {i} where
  ...
  module _ {A : Type i} where
    ...
    module FSMGElim {j} {P : FSMG A → Type j}
      ...
      (α* : {X Y Z : FSMG A} → (X* : P X) (Y* : P Y) (Z* : P Z) → ((X* ⊗* Y*) ⊗* Z*) == (X* ⊗* (Y* ⊗* Z*)) [ P ↓ α X Y Z ])
      (Λ* : {X : FSMG A} (X* : P X) → (I* ⊗* X*) == X* [ P ↓ Λ X ])
      (ρ* : {X : FSMG A} (X* : P X) → (X* ⊗* I*) == X* [ P ↓ ρ X ])
      (β* : {X Y : FSMG A} (X* : P X) (Y* : P Y) → (X* ⊗* Y*) == (Y* ⊗* X*) [ P ↓ β X Y ])
      (▽* : {X Y : FSMG A} (X* : P X) (Y* : P Y)
          → let p1 = (α* {X} {I} {Y} X* I* Y* ∙ᵈ $ (X* ⊗*_) (Λ* {Y} Y*)) in
            let p2 = $ (_⊗* Y*) (ρ* {X} X*) in
            p1 == p2 [ (λ p → ((X* ⊗* I*) ⊗* Y*) == (X* ⊗* Y*) [ P ↓ p ]) ↓ ▽ X Y ])
      ...
      (β²* : {X Y : FSMG A} (X* : P X) (Y* : P Y)
           → (β* {X} {Y} X* Y* ∙ᵈ β* {Y} {X} Y* X*) == idp [ (λ p → (X* ⊗* Y*) == (X* ⊗* Y*) [ P ↓ p ]) ↓ β² X Y ])
    ...
    module FSMGElimSet {j} {P : FSMG A → Type j} {{trunc* : {X : FSMG A} → has-level 0 (P X)}}
      ...
      private
        module E = FSMGElim {P = P} η* I* _⊗*_ α* Λ* ρ* β*
                   (λ _ _ → set-↓-has-all-paths-↓) (λ _ _ _ _ → set-↓-has-all-paths-↓) (λ _ _ _ → set-↓-has-all-paths-↓) (λ _ _ → set-↓-has-all-paths-↓)
                   (raise-level 0 trunc*)
    ...
    module FSMGRec {j} {P : Type j}
      ...
      private
        module E = FSMGElim {P = λ _ → P} η* I* _⊗*_
                            (λ X* Y* Z* → ↓-cst-in (α* X* Y* Z*))
                            (λ X* → ↓-cst-in (Λ* X*))
                            (λ X* → ↓-cst-in (ρ* X*))
                            (λ X* Y* → ↓-cst-in (β* X* Y*))
                            (λ X* Y* → TODO)
